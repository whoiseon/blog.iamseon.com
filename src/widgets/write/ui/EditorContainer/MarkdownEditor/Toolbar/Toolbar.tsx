...
const TOOLBAR_ITEM_CLASSNAME =
  'w-[2.5rem] h-[2.5rem] flex items-center justify-center text-[1.5rem] text-neutral-600 dark:text-neutral-400 cursor-pointer shrink-0 rounded-md bg-transparent dark:bg-transparent hover:bg-tag-bg-white hover:dark:bg-neutral-900 transition';
...
function Toolbar({ onClick = () => {}, innerRef }: Props) {
  return (
    <div
      id="toolbar"
      className="w-full top-0 flex items-center mb-[1rem] bg-transparent z-10 flex-wrap gap-x-1 -ml-2"
      ref={innerRef}
    >
      {[1, 2, 3, 4].map((number) => (
        <button
